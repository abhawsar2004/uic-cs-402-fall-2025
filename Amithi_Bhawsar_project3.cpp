...
vector<int> weird_traversal(TreeNode *root) {
  ...
  for (int i = odd_layers.size() - 1; i >= 0; --i) {
    ...
    for (int j = odd_layers[i].size() - 1; j >= 0; --j) {
      result.push_back(odd_layers[i][j]);
    }
  }
...
// Do not modify this fuction signature
TreeNode *bits_to_tree(const vector<bool> &bits) {
  // Wow, let's build a tree from secret codes!

  // First, let's check if the secret code is valid!
  // It must be even length!
  if (bits.size() % 2 != 0) {
    return nullptr;
  }

  // We need to count 1s and 0s to make sure they match!
  int ones = 0;
  int zeros = 0;
  for (bool b : bits) {
    if (b)
  ...
  // We use a stack to remember where we are!
  stack<TreeNode *> s;
  s.push(root);
...
pair<int, vector<int>> light_post_problem(int n, const vector<Edge> &edges,
  ...
  // Build the graph! Connect the dots!
  ...
  // DP tables!
  // dp[u][0]: u has light
  // dp[u][1]: u covered by child
  // dp[u][2]: u needs cover
  vector<vector<int>> dp(n, vector<int>(3));

  // To reconstruct the solution, we need to remember our choices!
  // But storing everything is hard. Let's just run DFS to compute costs,
  // and then run another DFS to pick the nodes! Smart!

  // We need to know parent to avoid going back up!
  // Let's do a post-order traversal using recursion!

  // Helper function for DP
  // We can't define function inside function easily in C++98/standard without
  // lambda, but we can use a separate helper or just a lambda if C++11 is
  // allowed (it is!). Or we can just implement it iteratively or with a helper
  // class. Let's use a lambda since we are using C++17 in the test command.

  // Wait, let's use a helper function or just recursion with a lambda.
  // Since we need to capture dp and adj, lambda is best.

  // We also need to handle the "force child" logic for state 1.

  // Let's use a struct for return type of DFS? No, we fill dp table.

  // We need to root the tree. Let's say root is 0.
  ...
  auto dfs = [&](auto &&self, int u, int p) -> void {
    ...
    // For state 1 (u covered by child), we need min(dp[v][0], dp[v][1]).
    // But if we pick dp[v][1] for ALL children, u is not covered!
    // So we might need to force one child to be dp[v][0].
    // We track the diff = dp[v][0] - min(dp[v][0], dp[v][1]).
    // We want to minimize sum + min_diff.
    // If we naturally picked a dp[v][0] (diff=0), then min_diff is 0.

    int min_diff = 1e9;
    bool any_child_has_light = false;
    ...
    for (int v : adj[u]) {
      ...
      // State 0: u has light. Children can be anything (covered by u).
      // v can be 0 (light), 1 (covered by child), 2 (needs cover).
      // dp[v][2] is valid because u covers v.
      sum_0 += min({dp[v][0], dp[v][1], dp[v][2]});

      // State 2: u needs cover. Children must be covered.
      // v can be 0 or 1.
      sum_2 += min(dp[v][0], dp[v][1]);

      // State 1: u covered by child.
      // v can be 0 or 1.
      int val = min(dp[v][0], dp[v][1]);
      sum_1 += val;

      if (dp[v][0] <= dp[v][1]) {
        any_child_has_light = true;
        min_diff = 0;
      } else {
        min_diff = min(min_diff, dp[v][0] - dp[v][1]);
      }
    }

    dp[u][0] = costs[u] + sum_0;
    dp[u][2] = sum_2;

    // If no children, state 1 is impossible (infinity).
    // But wait, if leaf, adj[u] size is 1 (parent).
    // If leaf (and not root), loop doesn't run.
    // sum_1 = 0. min_diff = 1e9.
    // dp[u][1] = 1e9. Correct.
    if (min_diff == 1e9)
      dp[u][1] = 1e9; // Large value
    else
      dp[u][1] = sum_1 + min_diff;
  };
  ...
  // state 0: u has light.
  // state 1: u covered by child.
  // state 2: u needs cover.

  auto reconstruct = [&](auto &&self, int u, int p, int state) -> void {
    if (state == 0) {
      lights.push_back(u);
    }
    ...
    if (state == 1) {
      for (int v : adj[u]) {
        ...
        if (dp[v][0] <= dp[v][1]) {
          ...
          if (dp[v][0] - dp[v][1] < min_diff) {
            min_diff = dp[v][0] - dp[v][1];
            best_v = v;
          }
        }
      }
    ...
    for (int v : adj[u]) {
      ...
      if (state == 0) {
        // We picked min(dp[v][0], dp[v][1], dp[v][2])
        int m = min({dp[v][0], dp[v][1], dp[v][2]});
        if (dp[v][0] == m)
          next_state = 0;
        else if (dp[v][1] == m)
          next_state = 1;
        ...
        // We picked min(dp[v][0], dp[v][1])
        int m = min(dp[v][0], dp[v][1]);
        if (dp[v][0] == m)
          next_state = 0;
        else
          next_state = 1;
      } else if (state == 1) {
        // We picked min(dp[v][0], dp[v][1]) but maybe forced one
        if (natural_light) {
          // We just pick min
          int m = min(dp[v][0], dp[v][1]);
          if (dp[v][0] == m)
          ...
          // We forced best_v to be 0
          if (v == best_v) {
            next_state = 0;
          } else {
            // Others are min(dp[v][0], dp[v][1]) which is dp[v][1] since
            // dp[v][0] > dp[v][1]
            next_state = 1;
          }
        }
  ...
  // If dp[0][2] is min, it's invalid for root (nobody covers it).
  // But dp[0][2] is "needs cover from parent". Root has no parent.
  // So we only compare 0 and 1.

  reconstruct(reconstruct, 0, -1, root_state);

  // Sort lights for output consistency (and test requirement?)
  sort(lights.begin(), lights.end());

  return {min(dp[0][0], dp[0][1]), lights};
}
...
smallest_subset_sum(unsigned int n, unsigned int t,
  ...
  // Let's find the smallest group of friends!

  // We need a big table to remember our answers!
  // dp[i][j] means the smallest number of items from index i to the end
  // that sum up to j.
  // We use a big number (1e9) to mean "impossible"!
  vector<vector<int>> dp(n + 1, vector<int>(t + 1, 1e9));

  // Base case: If we need sum 0, we take 0 items! Easy peasy!
  for (unsigned int i = 0; i <= n; ++i) {
    dp[i][0] = 0;
  }

  // Let's fill the table backwards! From the last item to the first!
  for (int i = n - 1; i >= 0; --i) {
    ...
    for (unsigned int j = 1; j <= t; ++j) {
      ...
      if (j >= val) {
        // If taking it gives a smaller size, we do it!
        if (dp[i + 1][j - val] != 1e9) {
  ...
  unsigned int current_target = t;

  // If the answer is impossible, return empty!
  if (dp[0][t] == 1e9) {
    return result;
  }
  ...
  for (unsigned int i = 0; i < n; ++i) {
    ...
    // If we take it, the remaining items (i+1 to end) must sum to
    // (current_target - val) AND the total size must match what we calculated
    // in dp[i][current_target]. We prioritize taking the current item if it
    // leads to the same minimum size because we want the lexicographically
    // smallest subset.
    if (current_target >= val && dp[i + 1][current_target - val] != 1e9) {
      if (1 + dp[i + 1][current_target - val] == dp[i][current_target]) {
        // Yes! Take it!
